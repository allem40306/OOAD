    ...
    public ArrayList<Dictionary<String, String>> searchCandidate(Dictionary<String, String> input){
     ...
     try {
      date = new SimpleDateFormat("yyyy-MM-dd")
              .parse(input.get("date"));
     } catch (ParseException e1) {
      // TODO Auto-generated catch block
      e1.printStackTrace();
     }
        DateFormat sdf = new SimpleDateFormat("HH:mm");
        String startStationID = input.get("start");
        String endStationID = input.get("end");
           for (int row = 0; row < 182; row++) {
            ...
            // check if the direction is right
            int dir = parseInt(startStationID) > parseInt(endStationID) ? 1 : 0;
            if(dir != parseInt(aTimeTable.get(row, "GeneralTimetable", "GeneralTrainInfo", "Direction")))
              continue;
            
            // check if the start and end station ID are in the range of this row's beginning and terminal station
            String trainStartStationID = aTimeTable.get(row, "GeneralTimetable", "GeneralTrainInfo", "StartingStationID");
            String trainEndStationID = aTimeTable.get(row, "GeneralTimetable", "GeneralTrainInfo", "EndingStationID");
            if((dir == 0 && parseInt(startStationID) >= parseInt(trainStartStationID) && parseInt(endStationID) <= parseInt(trainEndStationID)) ||
               (dir == 1 && parseInt(startStationID) <= parseInt(trainStartStationID) && parseInt(endStationID) >= parseInt(trainEndStationID)))
            {
             Dictionary<String, String> tmp = new Hashtable<String, String> ();
             String trainNo = aTimeTable.get(row, "GeneralTimetable", "GeneralTrainInfo", "TrainNo");
             String startTime = null;
          String endTime = null;
             for (int j = 0; j < aTimeTable.getNumOfStopTimes(row); j++)
             {
              if(startStationID.equals(aTimeTable.get(row, j, "GeneralTimetable", "StopTimes", "StationID")))
               startTime = aTimeTable.get(row, j, "GeneralTimetable", "StopTimes", "DepartureTime");
              if(endStationID.equals(aTimeTable.get(row, j, "GeneralTimetable", "StopTimes", "StationID")))
               endTime = aTimeTable.get(row, j, "GeneralTimetable", "StopTimes", "DepartureTime");
             }
               
//          aTimeTable.get(aTimeTable.getRowIndex(listOfValidTrainNo.get(i)[3]), j, "GeneralTimetable", "StopTimes", "DepartureTime");
          long difftime = 0;
       try {
        difftime = sdf.parse(endTime).getTime() - sdf.parse(startTime).getTime();
        long hour = difftime / 60000 / 60;
        long ms = difftime / 60000 % 60;
           String travelTime = Long.toString(hour) + ":" + Long.toString(ms); 
           tmp.put("train number", trainNo);
           tmp.put("departure time", startTime);
           tmp.put("arrival time", endTime);
           tmp.put("travel time", travelTime);
           dict.add(tmp);
       } catch (ParseException e) {
        // TODO Auto-generated catch block
        e.printStackTrace();
